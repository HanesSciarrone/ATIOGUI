...
ESP8266_StatusTypeDef_t ESP8266_ReceiveData(uint8_t *buffer, uint32_t *length)
{
	...
	*length = 0;
	strncpy((char *)bufferRx, "\0", MAX_BUFFER_SIZE);
	index = 0;
	newFrame = ESP8266_FALSE;

	while(1)
	{
		if( commInterface->recv(&charRx, 1) != 0 )
		{
			if( newFrame == ESP8266_TRUE )
			{
				if( lengthData-- )
				{
					*buffer++ = charRx;
		...
		else
		{
			...
			if (newFrame == ESP8266_TRUE && lengthData != 0)
			{
				return ESP8266_ERROR;
			}
			else
		...
		if(strstr((char *)bufferRx, (const char *)AT_STRING_IPD) != NULL && newFrame == ESP8266_FALSE)
		{
